...
describe OrdersController do
  orders_controller_actions = controller_actions("orders")
  ...
  describe "a user is not logged in" do
    orders_controller_actions.each do |action,req|
      if %w().include?(action)
        it "should reach the #{action} page" do
          send("#{req}", "#{action}", :id => @order.id)
  ...
  describe "a member is logged in" do
    before(:each) do
      ...
      @own_order = Factory(:order, :user_id => user.id )
    ...
    orders_controller_actions.each do |action,req|
      if %w().include?(action)
        ...
        it "should reach his own #{action} page" do
          send("#{req}", "#{action}", :id => @own_order.id)
