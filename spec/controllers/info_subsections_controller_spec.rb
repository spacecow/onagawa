...
describe InfoSubsectionsController do
  info_subsections_controller_actions = controller_actions("info_subsections")

  before(:each) do
    @info_subsection = Factory(:info_subsection)
  end

  describe "a user is not logged in" do
    info_subsections_controller_actions.each do |action,req|
      if %w().include?(action)
        it "should reach the #{action} page" do
          send("#{req}", "#{action}", :id => @info_subsection.id)
  ...
  describe "a member is logged in" do
    before(:each) do
      ...
      @own_info_subsection = Factory(:info_subsection, :user_id => user.id )
    ...
    info_subsections_controller_actions.each do |action,req|
      if %w().include?(action)
        ...
        it "should reach his own #{action} page" do
          send("#{req}", "#{action}", :id => @own_info_subsection.id)
