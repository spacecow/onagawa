...
describe MessagesController do
  messages_controller_actions = controller_actions("messages")
  ...
  describe "a user is not logged in" do
    messages_controller_actions.each do |action,req|
      if %w().include?(action)
        it "should reach the #{action} page" do
          send("#{req}", "#{action}", :id => @message.id)
  ...
  describe "a member is logged in" do
    before(:each) do
      ...
      @own_message = Factory(:message, :user_id => user.id )
    ...
    messages_controller_actions.each do |action,req|
      if %w().include?(action)
        ...
        it "should reach his own #{action} page" do
          send("#{req}", "#{action}", :id => @own_message.id)
